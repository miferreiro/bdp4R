...
proccess_files = function(pathFiles,
  ...
  if (!tools::file_ext(pathOutPut) %in% "csv" & !tools::file_ext(pathOutPut) %in% "RData") {
    stop("[proccess_files][Error]
  ...
  if (!tools::file_ext(pathOutPutSynsets) %in% "csv" & !tools::file_ext(pathOutPutSynsets) %in% "RData") {
    stop("[proccess_files][Error]
            Checking the extension of the variable: pathOutPutSynsets",
              tools::file_ext(pathOutPutSynsets))
  }
  ...
  if (tools::file_ext(pathOutPut) %in% "csv") {
    ...
    if (tools::file_ext(pathOutPut) %in% "RData") {
      saveRDS(get("dataFrame"), file = pathOutPut)
    } else {
      stop("[proccess_files][Error]
              Checking the extension of the variable: pathOutPut",
                tools::file_ext(pathOutPut))
    }
  }

  if (tools::file_ext(pathOutPutSynsets) %in% "csv") {
    ...
    if (tools::file_ext(pathOutPutSynsets) %in% "RData") {
      saveRDS(get("dataFrameAllSynsets"), file = pathOutPutSynsets)
    } else {
      stop("[proccess_files][Error]
              Checking the extension of the variable: pathOutPutSynsets",
                tools::file_ext(pathOutPutSynsets))
